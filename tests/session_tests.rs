...
async fn test_session_send_creation() {
    ...
    assert!(
        session.get_destination().is_none(),
...
async fn test_session_receive_creation() {
    ...
    let (session, token) = Session::new_receive(dest_path.clone(), key, nonce);
...
async fn test_session_claim_valid_token() {
    ...
    let (session, token) = Session::new_receive(dest_path, key, nonce);
...
async fn test_session_claim_invalid_token() {
    ...
    let (session, _token) = Session::new_receive(dest_path, key, nonce);
...
async fn test_session_get_file() {
    ...
    let (session, _token) = Session::new_send(manifest, key, nonce);
